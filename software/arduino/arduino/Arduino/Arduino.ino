    ...
    if (client) {
        ...
        while (client.connected()) {
            if (client.available()) {
                ...
                // Einde van HTTP-request
                if (c == '\n' && currentLineIsBlank) {

                    // Check of het een API-aanroep is
                    if (request.indexOf("GET /api/distance") >= 0) {
                        ...
                        client.print(distancePeak0 * .1);
                        client.println("}");

                    // Anders: serve HTML-pagina
                    } else {
                        client.println("HTTP/1.1 200 OK");
                        client.println("Content-Type: text/html");
                        client.println("Connection: close");
                        client.println();
                        client.println("<!DOCTYPE html><html><head><title>Radar Data</title>");
                        client.println("<style>body { font-family: Arial; text-align: center; padding-top: 50px; }</style>");
                        client.println("</head><body>");
                        client.println("<h1>Radar Afstand</h1>");
                        client.println("<h2 id='afstand'>Laden...</h2>");
                        client.println("<script>");
                        client.println("function updateAfstand() {");
                        client.println("  fetch('/api/distance')");
                        client.println("    .then(response => response.json())");
                        client.println("    .then(data => {");
                        client.println("      document.getElementById('afstand').innerText = data.distance + ' cm';");
                        client.println("    });");
                        client.println("}");
                        client.println("setInterval(updateAfstand, 200); updateAfstand();");
                        client.println("</script>");
