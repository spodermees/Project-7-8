...
{
	*value = PRESENCE_REG_APPLICATION_ID_ENUM_PRESENCE_DETECTOR;
}
...
void presence_reg_read_automatic_subsweeps(uint32_t *value)
{
	...
	bool bool_value = acc_detector_presence_config_automatic_subsweeps_get(config);
...
bool presence_reg_write_automatic_subsweeps(const uint32_t value)
{
	...
	{
		bool enabled = (value != 0);
		acc_detector_presence_config_automatic_subsweeps_set(config, enabled);
		status = true;
	}
...
{
	const acc_detector_presence_config_t *config     = i2c_presence_detector_get_config();
	float                                float_value = acc_detector_presence_config_signal_quality_get(config);
...
bool presence_reg_write_signal_quality(const uint32_t value)
{
	...
	{
		float float_value = acc_reg_protocol_uint32_milli_to_float(value);
		acc_detector_presence_config_signal_quality_set(config, float_value);
		status = true;
	}
